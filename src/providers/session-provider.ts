...
export class SessionProvider {
	...
	public init(): void {
		this.userProvider.fetch(false, true).then((data: any) => {
			this.ready = true;
			if (data) {
				this.setFirstDate(moment(data.reg_date, 'MMMM, DD YYYY HH:mm:ss ZZ', 'en').format());
			}
		});
	}
	...
	private populateSessions(): void {
		...
		let sessionStartDate: moment.Moment = moment(this.firstSessionDate).startOf('day');
		for (let iterator: number = 0; iterator < this.sessionDurations.length; iterator++) {
	...
	public getCurrentIndex(): number {
		if (
			...
			&& (
				...
				|| !this.lastCheckDate
				|| moment(this.lastCheckDate).format('YYYY-MM-DD') < moment().format('YYYY-MM-DD')
			)
		) {
			this.lastCheckDate = moment().format();

			this.currentSessionIndex = this.sessionStatuses.findIndex((item: any, index: number) => {
				let itemStartDate: moment.Moment = moment(item.startDate);
				let itemEndDate: moment.Moment = moment(item.startDate).add(this.sessionDurations[index], 'day');
				if (
	...
	public getCompletedSessions(): ISessionStatus[] {
		return this.sessionStatuses.filter((item: any) => item.status === SessionStatus.completed);
	}
	...
	public getPrevious(): ISessionStatus {
		...
		return this.sessionStatuses.find((session: ISessionStatus, index: number) => index === sessionIndex);
	}
